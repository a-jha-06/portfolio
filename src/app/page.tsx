...
import { getFeaturedProducts, getCaseStudies, getPRDs, isUsingSampleData } from "@/lib/content";

export default async function HomePage() {
  const [products, caseStudies, prds] = await Promise.all([
    getFeaturedProducts(),
    getCaseStudies(),
    getPRDs(),
  ]);
  ...
  return (
    <>
      ...
      <Section
        ...
        eyebrow="04 — Roadmaps"
        title="MVP → V2 → scale"
        description="Transparent scope shows you can ship incrementally and plan ahead."
        className="bg-white/[0.02]"
      >
        {products[0]?.roadmap ? (
          <div className="grid gap-4 md:grid-cols-3">
            {(
              [
                ["MVP", products[0].roadmap.mvp],
                ["V2", products[0].roadmap.v2],
                ["Future", products[0].roadmap.future],
              ] as const
            ).map(([label, items]) => (
              <div
                key={label}
                className="rounded-xl border border-white/8 p-5"
              >
                <h3 className="mb-3 font-semibold text-emerald-400">{label}</h3>
          ...
          <p className="text-zinc-500">Add roadmap data in CMS per product.</p>
        )}
      </Section>

      <Section
        id="competitive-analysis"
        eyebrow="05 — Competitive Analysis"
        title="Gaps & differentiators"
        description="Compare existing apps and show where your product wins."
      >
        {products[0]?.competitiveAnalysis ? (
          <div className="space-y-3">
            {products[0].competitiveAnalysis.map((row) => (
              <div
                key={row.name}
                className="rounded-xl border border-white/8 p-4 md:flex md:gap-6"
              >
                <div className="md:w-1/4 font-medium text-white">{row.name}</div>
                <div className="md:w-2/5 text-sm text-zinc-400">{row.gaps}</div>
                <div className="md:flex-1 text-sm text-emerald-300/90">
                  {row.differentiator}
                </div>
              </div>
            ))}
